...
```
Suspend-Check [-ApiKey] <string> [-BaseUrl] <string> [-UUID] <string> [<CommonParameters>]
```
...
Pauses a check in a Healthchecks instance using the API v3.
The check will stop accepting pings until resumed.
...
Returns success if the check is paused.
